...
function iEnhance (Repo) {
  const repo = prepRepo(Repo, 'products')
...
function itShouldAddANewProperty (t, err, actual) {
  t.ifError(err)
  t.strictEqual(actual.get(1).id, 1)
}

function itShouldOverwriteTheProperty (t, err, actual) {
  t.ifError(err)
  t.strictEqual(actual.create(), 'EVIL')
}

function itShouldThrowObjectIsNotExtensible (t, err) {
  t.strictEqual(err !== null, true, 'an error should be present')
  t.strictEqual(err.message.indexOf('object is not extensible') > -1, true)
}

function itShouldThrowCannotAssign (t, err) {
  t.strictEqual(err !== null, true, 'an error should be present')
  t.strictEqual(err.message.indexOf('Cannot assign to read only property') > -1, true)
}
